	...
	public void dbConnect() {
		try {
			...
			pstmtTotalScroll = conn.prepareCall(sqlSelect,
					...
					ResultSet.CONCUR_UPDATABLE); // ResultSet object의 변경이 가능 <=> ResultSet.CONCUR_READ_ONLY 읽기 전용 
...
//////Button Event ////////
	...
	public void del() {
		...
		if( strNo.length() <1 ) {
			JOptionPane.showMessageDialog(null, "번호는 필수 입니다. 입력해주세요");
			return;
		}
		
		String strName = null, strMail = null, strPhone = null;
		
		try {
			pstmtDelete.setString(1, strNo);
			int r = pstmtDelete.executeUpdate();
			if(r!=0) {
				JOptionPane.showMessageDialog(null, "삭제 했습니다");
			}else {
				JOptionPane.showMessageDialog(null, "해당 번호가 없습니다. 다시 입력해주세요");
				return;
			}
